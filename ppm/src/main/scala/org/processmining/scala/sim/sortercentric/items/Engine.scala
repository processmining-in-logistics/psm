...
import org.processmining.scala.viewers.spectrum2.pqr.SystemLayout
...
sealed class Engine(override val instance: AbstractSystem,
  ...
  private val sinkClient = new SinkClient("localhost", AppSettings(AppSettings.DefaultFileName).psmServerPort)
  private var prevEventsStartComplete = Map[String, (String, Long)]()
  private var prevEventsP = Map[String, (String, Long)]()
  private var prevEventsR = Map[String, (String, Long)]()
  ...
  def log(id: String, timestamp: Long, activity: String, flag: Int, lengthMs: Long) = {
    var ignore = false

    def send(id: String, activity: String, timestamp: Long, map: Map[String, (String, Long)], skipSending: Boolean = false) = {
      val prevOpt = map.get(id)
      if (prevOpt.isDefined) {
        if (isFirstPacket) {
          sinkClient.send(SegmentName("X", "Y"), createSegment(id, -1, -1))
          isFirstPacket = false
        }
        if (prevOpt.get._1 != activity) {
          if (!skipSending)
            sinkClient.send(SegmentName(prevOpt.get._1, activity), createSegment(id, prevOpt.get._2, timestamp))
          map + (id -> (activity, timestamp))
        } else {
          ignore = true
          map
        }
      } else map + (id -> (activity, timestamp))
    }

    prevEventsP = send(id, activity, timestamp, prevEventsP)
    if (!ignore) {
      val timestampStringStart = csvExportHelper.timestamp2String(timestamp)
      val timestampStringComplete = csvExportHelper.timestamp2String(timestamp + lengthMs)
      val a1 = s"$activity${SystemLayout.Sep}${SystemLayout.Start}"
      eventLog(s""""$id", $timestampStringStart,$a1,$flag""")
      prevEventsStartComplete = send(id, a1, timestamp, prevEventsStartComplete)
      prevEventsR = send(activity, a1, timestamp, prevEventsR)
      val a2 = s"$activity${SystemLayout.Sep}${SystemLayout.Complete}"
      eventLog(s""""$id", $timestampStringComplete,$a2,$flag""")
      prevEventsStartComplete = send(id, a2, timestamp + lengthMs, prevEventsStartComplete)
      prevEventsR = send(activity, a2, timestamp + lengthMs, prevEventsR, true)
      ...
      if (!canMove) newStatePhase1TimeUpdated.copy(wasMovedDuringLastStep = false)
        ...
        if (tsuAndOutgoingTsu._2.isDefined) {
          ...
          log(tsuAndOutgoingTsu._2.get.tsu.id, newStatePhase1TimeUpdated.timeMs, s"${thisConveyor.id}${Separator.S}${thatConveyor.id}", Engine.ExtendedMoveEvent,
            tsuAndOutgoingTsu._2.get.tsu.lengthMsOnConveyor(thisConveyor.config))
        }
  ...
  def logSensors(thisConveyor: AbstractConveyor): Unit = {
    ...
    def logSensor(sensorConfig: SensorConfig, tsuState: TsuState, flag: Int) = {
      ...
      if (distanceMs >= 0 && distanceMs < stepMs) {
        log(tsuState.tsu.id, instance.state.timeMs, s"${sensorConfig.id}", flag, tsuState.tsu.lengthMsOnConveyor(thisConveyor.config))
        //log(tsuState.tsu.id, instance.state.timeMs - tsuState.backPositionMs(thisConveyor.config), s"${sensorConfig.id}_c", flag)
      }
    }
      ...
      if (!canMove) newStateTimeUpdated.copy(wasMovedDuringLastStep = false)
        ...
        val finalTsu = if (outgoingTsu.isDefined) {
          ...
          log(tsuToBeMerged.tsu.id, newStateTimeUpdated.timeMs, s"${thisConveyor.id}${Separator.S}${thatConveyor.id}", Engine.StandardMergeEvent,
            tsuToBeMerged.tsu.lengthMsOnConveyor(thisConveyor.config))
          newTsu.init
        } else newTsu
        ...
        if (isReadyToReceive) {
          ...
          if (tsuOptional.isDefined) {
            ...
            log(tsuOptional.get.tsu.id, instance.state.timeMs, s"${x.sensorId}", Engine.StandardDivertEvent,
              tsuOptional.get.tsu.lengthMsOnConveyor(thatConveyor.config))
