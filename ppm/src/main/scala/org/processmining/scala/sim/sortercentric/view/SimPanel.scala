...
import org.processmining.scala.viewers.spectrum2.model.{Segment, SegmentEventImpl, SegmentName, SinkClient}
...
class SimPanel(experimentEnv: ExperimentTemplate) extends BorderPanel {
  ...
  def executeStep()= {
    val currenttime = engine.executeStep()
    interactiveModelPanel.forceRepaint()
    psmMouseMoveListener(engine.instance.state.timeMs)
    if(currenttime - prevTimeStep > 1000) {
      sinkClient.send(SegmentName("Update", "SimTime"), Segment("id", SegmentEventImpl(currenttime), SegmentEventImpl(currenttime), 0))
