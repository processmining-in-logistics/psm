...
object SegmentsToSpectrum {
  ...
  def start(filenames: List[String],
    ...
    val session = PreprocessingSession(startTimeMs, startTimeMs + twCount * twSizeMs, twSizeMs, durationClassifier.classCount, "start, pending, end", durationClassifier.toString)
    ...
    logger.info(s"$spectrumRootDir/${SegmentProcessor.SessionFileName} is created.")

    filenames
      .map(new File(_))
      .foreach(_.delete)
    logger.info("Segment files are deleted.")
  }
}
