...
import org.processmining.scala.viewers.spectrum2.model.{AbstractDataSource, CallableWithLevel, SegmentName, SinkDatasourceBuilder}
...
class PsmMainFrame(eventLogPropertiesFactory: EventLogPropertiesFactory, builderFactory: BuilderFactory, onlyObservedPs: Boolean) extends BorderPanel {
  ...
  var sinkBuilder = new CallableWithLevel[AbstractDataSource] {
    private val sl = SystemLayout()

    override def call(level: Int): AbstractDataSource = SinkDatasourceBuilder(sl.psmSegments.toVector, sl.initialSortingOrder.toVector)

    override def levels: Vector[String] = Vector(PsmMainFrame.ObservedPsText)
  }


  def updateHorizontalScrollAndZoom(posScroll: Int, horizontalZoom: Int): Unit = {
    horizontalScrollSlider.value = posScroll
    horizontalZoomSlider.value = horizontalZoom
  ...
  val controlPanel = new BorderPanel {
    ...
    val middleAndBottomOfControlPanel = new BorderPanel {
      ...
      val checkBoxShowOngoing = new CheckBox("Ongoing") {
        selected = false
        listenTo(this)
        reactions += {
          case ButtonClicked(_) => psm.onShowOngoing(selected)
        }
      }
      ...
      val openButton = new Button("Start")
      ...
      reactions += {
        ...
        case evt@MouseClicked(`openButton`, pt, _, _, _) => {
          if (evt.peer.getButton == 1) {
            ...
            psm.onVerticalZoomChanged(psm.verticalZoom)
            levelBox.repaint()
            //peer.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR))
      ...
      val bottomPanel = new FlowPanel(FlowPanel.Alignment.Trailing)(
        levelBox, checkBoxShowReources, checkBoxShowRightBorders, checkBoxColorByOrigin, checkBoxShowOngoing, overlaidSegmentsBox,
